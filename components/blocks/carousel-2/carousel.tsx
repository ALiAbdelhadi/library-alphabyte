...
export const Carousel = ({ items, initialScroll = 0, cardGap = 16, scrollOffset = 300 }: CarouselProps) => {
   ...
   return (
      <CarouselContext.Provider value={{ onCardClick: handleCarsouelClick, currentIndex }}>
         <div className="relative w-full py-0 !my-12 ">
            ...
            <div className="flex w-full overflow-x-scroll py-6 scroll-smooth  [scrollbar-width:none]" ref={carouselRef} onScroll={checkScrollAbility}>
               <div className="flex flex-row justify-end" style={{ gap: `${cardGap}px` }}>
                  {items.map((item, index) => (
                     <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.5, delay: 0.3 * index } }}>
                        {item}
                     </motion.div>
                  ))}
               </div>
